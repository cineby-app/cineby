...
// ========== EPISODE DETAIL POPUP — restored from your original ==========
          ...
          <div className="relative flex max-h-[62dvh] flex-col overflow-y-auto bg-[#05050A] p-3 sm:p-6 md:p-8 lg:max-h-none lg:overflow-visible lg:p-9 custom-scrollbar">
            ...
            <div className="grid grid-cols-3 gap-2 lg:mt-8 lg:grid-cols-1 xl:grid-cols-3 xl:gap-3">
              <div className="rounded-xl border border-white/10 bg-white/[0.04] p-2.5 sm:p-4">
                <p className="text-[8px] uppercase tracking-widest text-gray-500">Date</p>
                <p className="mt-1 text-[11px] font-bold text-white sm:text-sm">{shortAirDate}</p>
              </div>
              <div className="rounded-xl border border-white/10 bg-white/[0.04] p-2.5 sm:p-4">
                <p className="text-[8px] uppercase tracking-widest text-gray-500">Season</p>
                <p className="mt-1 text-[11px] font-bold text-white sm:text-sm">S{selectedSeason}</p>
              </div>
              <div className="rounded-xl border border-white/10 bg-white/[0.04] p-2.5 sm:p-4">
                <p className="text-[8px] uppercase tracking-widest text-gray-500">Episode</p>
                <p className="mt-1 text-[11px] font-bold text-white sm:text-sm">E{episode.episode_number}</p>
              </div>
            </div>
            ...
            <div className="mt-3 flex gap-2 sm:mt-5 sm:flex-row sm:gap-3 lg:mt-auto lg:pt-6">
              <ContentLocker
                movieId={seriesId}
                movieTitle={`${seriesName} - S${selectedSeason}E${episode.episode_number}`}
                backdrop={episode.still_path || undefined}
                mediaType="tv"
                season={selectedSeason}
                episode={episode.episode_number}
                overrideUrl={`https://movieplex.online/watch/tv/${seriesId}/${selectedSeason}/${episode.episode_number}`}
                customTrigger={
                  <button className="flex flex-1 items-center justify-center gap-2 rounded-full bg-gradient-to-r from-[#E50914] to-[#b50000] px-4 py-3 text-xs font-black uppercase tracking-wider text-white shadow-lg shadow-[#E50914]/25 transition-all hover:scale-[1.02] hover:from-[#b50000] hover:to-[#7f0000] sm:px-6 sm:py-3.5 sm:text-sm">
...
function EpisodesModal({
  ...
  useEffect(() => {
    async function loadEpisodes() {
      ...
      try {
        const data = await fetchTVSeasonEpisodes(seriesId.toString(), selectedSeason);
        const episodeList =
  ...
  return createPortal(
    ...
    >
      <div
        className="relative w-full max-w-5xl max-h-[96dvh] overflow-hidden rounded-2xl md:rounded-3xl border border-[#E50914]/40 bg-[#05050A] shadow-[0_0_100px_rgba(229,9,20,0.18)]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        ...
        <div className="max-h-[calc(96dvh-118px)] overflow-y-auto custom-scrollbar">
          ...
          {/* Episodes — card under card, with Watch Now below each card */}
            ...
            {loadingEpisodes ? (
              ...
              <div className="flex flex-col gap-4">
                {episodes.map((episode) => {
                  ...
                  return (
                    <div
                      key={episode.id || `${selectedSeason}-${episode.episode_number}`}
                      className="rounded-2xl border border-white/10 bg-white/[0.04] p-3 sm:p-4 transition-all hover:border-[#E50914]/50 hover:bg-white/[0.06]"
                    >
                      {/* Episode card */}
                      ...
                      {/* ✅ Watch Now button UNDER the card */}
                      <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-white/10">
                        <ContentLocker
                          ...
                          overrideUrl={`https://movieplex.online/watch/tv/${seriesId}/${selectedSeason}/${episode.episode_number}`}
                          customTrigger={
                            <button className="w-full flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-[#E50914] to-[#b50000] hover:from-[#b50000] hover:to-[#7f0000] px-5 py-3 text-xs sm:text-sm font-black uppercase tracking-wider text-white transition-all shadow-lg shadow-[#E50914]/25 hover:scale-[1.01]">
        ...
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="text-sm text-gray-400">
              {validSeasons.length} season{validSeasons.length !== 1 ? "s" : ""} • {totalEpisodes} episode{totalEpisodes !== 1 ? "s" : ""}
            </p>
          </div>
          <button
  ...
  return (
    <div className="relative w-full overflow-hidden">
      <div className="overflow-x-auto overflow-y-visible pb-4 scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none', WebkitOverflowScrolling: 'touch' }}>
        <div className="flex gap-4 md:gap-5 w-max px-0.5">{children}</div>
      </div>
          ...
          return (
            <Link href={`/tv/${slug}`} key={show.id} className="group relative block rounded-lg md:rounded-xl overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-xl">
              <div className="aspect-[2/3] relative bg-gradient-to-br from-[#1F2937] to-[#0F0F1A]">
                {posterPath ? (
                  <Image src={`https://image.tmdb.org/t/p/w342${posterPath}`} alt={showName} fill className="object-cover transition-transform duration-500 group-hover:scale-110" referrerPolicy="no-referrer" />
                ) : (
...
export default function TVPage({ params }: { params: Promise<{ tvSlug: string }> }) {
  ...
  // State to open Seasons & Episodes modal from hero "Watch Now"
  const [seasonsModalOpen, setSeasonsModalOpen] = useState(false);
  const [seasonsInitialSeason, setSeasonsInitialSeason] = useState(1);
    ...
    return (
      <main className="min-h-screen bg-black flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          <div className="w-12 h-12 border-4 border-[#1F2937] border-t-[#E50914] rounded-full animate-spin" />
  ...
  const openSeasonsModal = (seasonNumber: number) => {
    setSeasonsInitialSeason(seasonNumber);
  ...
  return (
    <main className="min-h-screen bg-black text-white selection:bg-[#E50914] selection:text-white">
        ...
        <div className="absolute bottom-0 left-0 w-full px-4 sm:px-6 md:px-16 lg:px-24 pb-8 sm:pb-8 md:pb-12 z-10">
          <div className="flex flex-col md:flex-row gap-4 sm:gap-6 md:gap-10 items-start md:items-end max-w-7xl mx-auto">
            ...
            <div className="flex-1 space-y-3 sm:space-y-4 md:space-y-5 pb-2 sm:pb-4">
              ...
              {/* ✅ Hero "Watch Now" → opens Seasons & Episodes modal */}
              <div className="flex flex-wrap items-center gap-2 sm:gap-3 pt-2 sm:pt-4">
                <button
                  onClick={() => openSeasonsModal(validSeasons[0]?.season_number || 1)}
      ...
      {/* ✅ Seasons modal opened from hero "Watch Now" */}
      {seasonsModalOpen && validSeasons.length > 0 && (
        <EpisodesModal
          seasons={validSeasons}
          seriesId={showId}
          seriesName={showName}
          initialSeason={seasonsInitialSeason}
