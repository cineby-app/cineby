  ...
  return (
    <div className="relative w-full">
      <div className="overflow-x-auto scrollbar-hide pb-4" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        <div className="flex gap-5 min-w-max">{children}</div>
      </div>
...
function RecommendedMovies({ movies }: { movies: any[] }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3"><div className="w-1 h-8 bg-[#E50914] rounded-full" /><h2 className="text-xl md:text-2xl font-bold tracking-tight">Recommended for You</h2><span className="text-sm text-gray-500">Top picks</span></div>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {topMovies.map((movie) => {
          const slug = `${movie.title.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-${movie.id}`;
          const year = movie.release_date ? new Date(movie.release_date).getFullYear() : '';
          return (
            <Link href={`/${slug}`} key={movie.id} className="group relative block rounded-xl overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-xl">
              <div className="aspect-[2/3] relative bg-gradient-to-br from-[#1F2937] to-[#0F0F1A]">
                {movie.poster_path ? (<Image src={`https://image.tmdb.org/t/p/w342${movie.poster_path}`} alt={movie.title} fill className="object-cover" referrerPolicy="no-referrer" />) : (<div className="w-full h-full flex items-center justify-center"><svg className="w-12 h-12 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 4v16M17 4v16M3 8h4m10 0h4M3 12h18M3 16h4m10 0h4M4 20h16a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1 1v14a1 1 0 001 1z" /></svg></div>)}
                <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-3">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <div className="flex items-center gap-1"><svg className="w-3 h-3 text-[#E50914]" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" /></svg><span className="text-[#E50914] text-xs font-bold">{movie.vote_average.toFixed(1)}</span></div>
...
export default function MoviePage({ params }: { params: Promise<{ movieSlug: string }> }) {
  ...
  useEffect(() => {
    ...
    async function loadReviews() {
      ...
      setReviews(reviewsData.results.slice(0, 3)); // Show top 3 reviews
    ...
    <main className="min-h-screen bg-black text-white selection:bg-[#E50914] selection:text-white">
      ...
      <div className="relative w-full h-[70vh] md:h-[80vh] min-h-[550px] overflow-hidden">
        ...
        <div className="absolute inset-0 z-0 pointer-events-none">
          ...
          <div className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-[#E50914]/10 rounded-full blur-3xl animate-pulse delay-1000" />
        </div>
        <div className="absolute bottom-0 left-0 w-full px-6 md:px-16 lg:px-24 pb-12 md:pb-16 z-10">
          <div className="flex flex-col md:flex-row gap-6 md:gap-10 items-end max-w-7xl mx-auto">
            <div className="hidden md:block group relative cursor-pointer" onClick={() => handleImageClick(0)}>
              <div className="w-56 lg:w-64 aspect-[2/3] rounded-2xl overflow-hidden shadow-2xl transform transition-all duration-500 group-hover:scale-105 group-hover:shadow-[0_0_40px_rgba(229,9,20,0.4)]">
                <Image
              ...
              <div className="absolute -bottom-3 -right-3 bg-black/90 backdrop-blur-md rounded-full px-4 py-2 border border-[#E50914]/50 shadow-lg">
                <div className="flex items-center gap-2">
                  <svg className="w-5 h-5 text-[#E50914] fill-current" viewBox="0 0 20 20">
            ...
            <div className="flex-1 space-y-5 pb-4">
              <div className="text-sm text-gray-400">
                ...
                <span className="mx-2">/</span>
                <span className="text-[#E50914]">{movie.title}</span>
              </div>
              <h1 className="text-4xl md:text-6xl lg:text-7xl font-black tracking-tighter leading-[1.1] bg-gradient-to-r from-white via-white to-gray-400 bg-clip-text text-transparent">
                {movie.title}
              </h1>
              ...
              <div className="flex flex-wrap gap-2">
                {movie.genres?.slice(0, 5).map((g: any) => (
                  <Link key={g.id} href={`/genre/${g.id}?name=${encodeURIComponent(g.name)}`}
                    className="px-4 py-2 rounded-full text-xs font-semibold uppercase tracking-wider bg-white/10 backdrop-blur-sm border border-white/20 hover:bg-[#E50914] hover:border-[#E50914] transition-all duration-300 hover:scale-105">
                    {g.name}
                  </Link>
                ))}
              </div>
      ...
      {/* Main Content */}
      <div className="w-full max-w-7xl mx-auto px-6 md:px-16 lg:px-24 mt-4 relative z-20">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12">
          <div className="lg:col-span-2 space-y-10">
            ...
            {cast.length > 0 && (
              <div className="space-y-5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3"><div className="w-1 h-8 bg-[#E50914] rounded-full" /><h2 className="text-xl md:text-2xl font-bold tracking-tight">Top Cast</h2></div>
                  <span className="text-xs text-gray-500">{cast.length} actors</span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {cast.slice(0, 10).map((person) => (
                    <Link href={`/actor/${slugify(person.name, person.id)}`} key={person.id} className="group flex items-center gap-4 p-4 bg-gradient-to-br from-[#0F0F1A] to-black rounded-xl border border-[#1F2937] hover:border-[#E50914]/50 transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5">
                      <div className="w-14 h-14 rounded-full overflow-hidden bg-gradient-to-br from-[#1F2937] to-[#0F0F1A] border-2 border-[#1F2937] group-hover:border-[#E50914] transition-all duration-300 shrink-0">
                        {person.profile_path ? (<Image src={`https://image.tmdb.org/t/p/w185${person.profile_path}`} alt={person.name} width={56} height={56} className="w-full h-full object-cover" referrerPolicy="no-referrer" />) : (<div className="w-full h-full flex items-center justify-center"><svg className="w-7 h-7 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg></div>)}
                      </div>
                      <div className="flex-1 min-w-0"><p className="text-base font-bold text-white group-hover:text-[#E50914] transition-colors truncate">{person.name}</p><p className="text-xs text-gray-400 truncate">as {person.character}</p></div>
                      <svg className="w-4 h-4 text-gray-600 group-hover:text-[#E50914] group-hover:translate-x-1 transition-all" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                    </Link>
                  ))}
                </div>
              </div>
            ...
            {filteredCrew.length > 0 && (
              <div className="space-y-5">
                <div className="flex items-center gap-3"><div className="w-1 h-8 bg-[#E50914] rounded-full" /><h2 className="text-xl md:text-2xl font-bold tracking-tight">Crew & Production</h2></div>
                <HorizontalScroller>
                  {filteredCrew.map((person, idx) => (
                    <Link href={getRoleHref(person)} key={`${person.id}-${idx}`} className="group flex flex-col w-36 shrink-0 bg-gradient-to-b from-[#0F0F1A] to-black rounded-xl border border-[#1F2937] overflow-hidden hover:border-[#E50914]/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
                      <div className="w-full aspect-[2/3] relative bg-gradient-to-br from-[#1F2937] to-[#0F0F1A] overflow-hidden">
                        {person.profile_path ? (<Image src={`https://image.tmdb.org/t/p/w185${person.profile_path}`} alt={person.name} fill className="object-cover group-hover:scale-110 transition-transform duration-500" referrerPolicy="no-referrer" />) : (<div className="w-full h-full flex items-center justify-center"><svg className="w-12 h-12 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg></div>)}
                        <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                      </div>
                      <div className="p-3 text-center">
                        <p className="text-sm font-bold text-white group-hover:text-[#E50914] transition-colors line-clamp-1">{person.name}</p>
                        <p className="text-[10px] text-gray-400 mt-1 line-clamp-2">{person.job}</p>
                        <p className="text-[9px] text-[#E50914] font-mono mt-1 uppercase tracking-wider">{person.department}</p>
                      </div>
                    </Link>
                  ))}
                </HorizontalScroller>
            ...
            {images.backdrops.length > 0 && (
              <div className="space-y-5">
                <div className="flex items-center gap-3"><div className="w-1 h-8 bg-[#E50914] rounded-full" /><h2 className="text-xl md:text-2xl font-bold tracking-tight">Media Gallery</h2></div>
                <HorizontalScroller>
                  {images.backdrops.slice(0, 10).map((img: any, idx: number) => (
                    <div key={idx} onClick={() => handleImageClick(1 + idx)} className="group relative w-80 shrink-0 aspect-video rounded-xl overflow-hidden border border-[#1F2937] bg-[#0F0F1A] cursor-pointer hover:scale-105 transition-all duration-300 hover:shadow-2xl hover:border-[#E50914]/50">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-[#0F0F1A] to-black rounded-2xl border border-[#1F2937] p-6 space-y-5">
              <div className="flex items-center gap-2 pb-2 border-b border-[#1F2937]">
                <svg className="w-5 h-5 text-[#E50914]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                <h3 className="text-sm font-bold tracking-widest text-gray-400 uppercase">Movie Info</h3>
              </div>
              <div className="grid grid-cols-2 gap-4">
                {movie.status && (<div><p className="text-xs text-gray-500 uppercase tracking-wider">Status</p><p className="text-sm font-semibold text-white mt-1">{movie.status}</p></div>)}
                {movie.original_language && (<div><p className="text-xs text-gray-500 uppercase tracking-wider">Language</p><p className="text-sm font-semibold text-white uppercase mt-1">{movie.original_language}</p></div>)}
                {movie.release_date && (<div><p className="text-xs text-gray-500 uppercase tracking-wider">Release Date</p><p className="text-sm font-semibold text-white mt-1">{new Date(movie.release_date).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</p></div>)}
                {movie.runtime && (<div><p className="text-xs text-gray-500 uppercase tracking-wider">Runtime</p><p className="text-sm font-semibold text-white mt-1">{Math.floor(movie.runtime / 60)}h {movie.runtime % 60}m</p></div>)}
              </div>
            </div>
              ...
              <div className="bg-gradient-to-br from-[#0F0F1A] to-black rounded-2xl border border-[#1F2937] p-6">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-bold tracking-widest text-gray-400 uppercase">User Reviews</h3>
                  </div>
                  {totalReviews > 3 && (
                    <button onClick={() => setReviewsModalOpen(true)} className="text-xs text-[#E50914] hover:text-white transition">See All ({totalReviews})</button>
                  )}
                </div>
                <div className="space-y-4">
                  {reviews.map((review) => (
                    <div key={review.id} className="border-b border-[#1F2937] pb-4 last:border-0">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="w-6 h-6 rounded-full bg-[#1F2937] flex items-center justify-center">
                          <span className="text-white text-xs font-bold">{review.author.charAt(0).toUpperCase()}</span>
                        </div>
                        <span className="text-white text-sm font-medium">{review.author}</span>
                        {review.author_details.rating && (
                          <div className="ml-auto flex items-center gap-1">
                            <span className="text-yellow-500 text-xs">★</span>
                            <span className="text-white text-xs">{review.author_details.rating}/10</span>
                          </div>
                        )}
                      </div>
                      <p className="text-gray-300 text-sm line-clamp-3">{review.content}</p>
                      <p className="text-gray-500 text-xs mt-2">{formatReviewDate(review.created_at)}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {keywords.length > 0 && (
              <div className="bg-gradient-to-br from-[#0F0F1A] to-black rounded-2xl border border-[#1F2937] p-6">
                <div className="flex items-center gap-2 pb-3 border-b border-[#1F2937] mb-4">
                  <svg className="w-5 h-5 text-[#E50914]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14" /></svg>
                  <h3 className="text-sm font-bold tracking-widest text-gray-400 uppercase">Keywords</h3>
                </div>
                <div className="flex flex-wrap gap-2">
                  {keywords.slice(0, 15).map((kw: any) => (
                    <Link key={kw.id} href={`/keyword/${kw.name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-${kw.id}`} className="px-3 py-1.5 rounded-full bg-white/5 text-gray-300 text-xs font-medium hover:bg-[#E50914] hover:text-white transition-all duration-300 hover:scale-105">#{kw.name.replace(/\s/g, '')}</Link>
                  ))}
                </div>
              </div>
            )}
      ...
      {related.length > 0 && (
        <div className="mt-16 px-6 md:px-16 lg:px-24 pb-16">
      ...
      {trailerOpen && trailerKey && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 sm:p-8 bg-black/90 backdrop-blur-md" onClick={() => setTrailerOpen(false)}>
          <div className="relative w-full max-w-6xl aspect-video bg-black rounded-2xl overflow-hidden border border-[#BE185D]/50" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setTrailerOpen(false)} className="absolute top-4 right-4 z-10 w-12 h-12 bg-black/60 hover:bg-[#BE185D] text-white rounded-full flex items-center justify-center transition-all border border-gray-600 hover:border-[#BE185D] backdrop-blur-sm" aria-label="Close Trailer">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
