        ...
        <div className="p-4 bg-[#08080d]">
          <div className="flex gap-4">
            ...
            <div className="min-w-0 flex-1">
              {/* ✅ Popup is aria-hidden, so no heading pollution */}
              <p className="line-clamp-2 text-lg sm:text-xl font-black text-white">
                {title}
              </p>
      ...
      <div className="relative w-full overflow-hidden">
        ...
        >
          {doubledItems.map((item, idx) => {
            ...
            const href = `/${slug}`;
            // ✅ Detect duplicate half (second copy from doubledItems)
            const isDuplicate = idx >= top10Items.length;

            return (
              <Link
                href={href}
                key={`${item.id}-${idx}`}
                // ✅ Hide duplicate from Google + screen readers
              ...
              >
                ...
                <div className="mt-2 sm:mt-3">
                  {/* ✅ H3 for movie title + sr-only keywords */}
                  <h3 className="text-xs sm:text-sm font-bold text-white truncate group-hover:text-[#E50914] transition-colors">
                    {title}
                    <span className="sr-only"> - Watch free in Full HD on Cineby</span>
                  </h3>
                  {/* ✅ H4 for metadata */}
                  <h4 className="flex items-center gap-2 text-[10px] sm:text-xs text-gray-400 mt-0.5">
      ...
      <div className="relative w-full overflow-visible">
        ...
        >
          {doubledItems.map((item, idx) => {
            ...
            // ✅ Detect duplicate half
            const isDuplicate = idx >= displayItems.length;
          ...
          {[...displayItems, ...displayItems].map((item, idx) => {
            ...
            return (
              ...
              >
                ...
                {/* ✅ H3 + H4 overlay (only visible on hover visually, but present in DOM) */}
                <div className="absolute bottom-0 left-0 right-0 p-2 z-10 pointer-events-none opacity-0 group-hover/card:opacity-100 transition-opacity duration-300 bg-gradient-to-t from-black/90 to-transparent">
                  <h3 className="text-[11px] font-bold text-white truncate">
                    {displayTitle}
                    <span className="sr-only"> - Watch free in Full HD on Cineby</span>
                  </h3>
                  <h4 className="flex items-center gap-1.5 text-[9px] text-gray-400 mt-0.5">
                    <span>{year}</span>
                    <span>•</span>
                    <span>{isMovie ? "Movie" : "TV Show"}</span>
                  </h4>
...
export function InfiniteMovieRows({
  ...
  const trendingMovies = hasMovies ? movies.slice(0, ITEMS_PER_ROW) : [];
  const popularMovies = hasMovies ? movies.slice(ITEMS_PER_ROW, ITEMS_PER_ROW * 2) : [];
  const topRatedMovies = hasMovies ? movies.slice(ITEMS_PER_ROW * 2, ITEMS_PER_ROW * 3) : [];

  const trendingTV = hasTvShows ? tvShows.slice(0, ITEMS_PER_ROW) : [];
  const popularTV = hasTvShows ? tvShows.slice(ITEMS_PER_ROW, ITEMS_PER_ROW * 2) : [];
  const topRatedTV = hasTvShows ? tvShows.slice(ITEMS_PER_ROW * 2, ITEMS_PER_ROW * 3) : [];
